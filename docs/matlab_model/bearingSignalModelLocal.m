...
# Define the geometry parameter for given fault type
if nargin < 14
    qAmpMod = 1;
end
switch faultType
...
# Geometry pamerter not including n_balls is not consistent with paper. n_balls incorporated later which could be confusing.

# Get the average angular distance that should be rotated until an impulse occurs
Ltheta = length(fr); # The length of the speed profile vector.  Speed profile vector seems to be given as function of angle.
theta = (0:Ltheta-1)*2*pi/N # Discretisation every angle traversed from start to finish of simulation 0 -> 2pi*nrevs: Technically specified by user?
...
frThetaFault = interp1(theta,fr,thetaFault,'spline'); # Get the rotation frequencies at specifically the angular positions where an impulse will occur
...
frTime = interp1(tTimp,frThetaFault,t,'spline'); % The rotation frequency as a function of time. Why define rotation speed initially as function of angle?
...
if strcmp(faultType,'inner')
    if length(fc) > 1, # If the length of the carrier component of the speed is not a scalar:
        ...
        for index = 2:length(fr),
            thetaTime(index) = thetaTime(index - 1) + (2*pi/N)/(2*pi*fr(index)); # This is essentially integration by Eulers method? Finding the angle for a given sample time
        end
...
x = fftfilt(sdofRespTime,x); # This function should give the same result as conv, but should be faster.
